...
export default class RoomClient {
	...
	// 			externalVideo)
	// 	{
	// 		console.log('cons',roomId,	peerId,
	// 				displayName,
	// 				device,
	// 				handlerName,
	// 				useSimulcast,
									...
									case 'chat':
										{
											...
											store.dispatch(stateActions.setTemperature(
												{
													temperature: message, peerId: sendingPeer.id
												}
											))
	...
	async enableMic() {
		...
		try {
			if (!this._externalVideo) {
				...
				// console.log('myProps', true, track);
			}
			else {
				const stream = await this._getExternalVideoStream();

				track = stream.getAudioTracks()[0].clone();
				// console.log('myProps', false, track);
			}
	...
	async sendChatMessage(text) {
		...
		try {
			const { appData, _appData, send } = this._chatDataProducer;
			...
			// ['1jhg35y-jhwegs47', '1jhg35y-jhwegs47'] => Req
			...
			console.log('myProps', this._chatDataProducer);
			//...
